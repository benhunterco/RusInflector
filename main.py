...
if __name__ == "__main__":
    ...
    while(not word == "exit"):
        ...
        if(parsed_word.tag.__contains__("VERB") or parsed_word.tag.__contains__("INFN")):
            ...
            print("Nominative: " + parsed_word.inflect({"nomn"}).word + " Plural: " + parsed_word.inflect({"nomn", "plur"}).word)
            print("Prepositional: " + parsed_word.inflect({"loct"}).word + " Plural: " + parsed_word.inflect({"loct", "plur"}).word)
            print("Accusative: " + parsed_word.inflect({"accs"}).word + " Plural: " + parsed_word.inflect({"accs", "plur"}).word)
            print("Genitive: " + parsed_word.inflect({"gent"}).word + " Plural: " + parsed_word.inflect({"gent", "plur"}).word)
            print("Instrumental: " + parsed_word.inflect({"ablt"}).word + " Plural: " + parsed_word.inflect({"ablt", "plur"}).word)
            print("Dative: " + parsed_word.inflect({"datv"}).word + " Plural: " + parsed_word.inflect({"datv", "plur"}).word)
        word = input("Word: ")